...
test.describe(`${MODULE_NAME} — Giao diện`, () => {
  test('UM-UI-001: Trang danh sách tải đúng bố cục', async ({ page }) => {
  ...
  test('UM-UI-002: Trang tạo mới hiển thị đầy đủ trường', async ({ page }) => {
...
test.describe(`${MODULE_NAME} — Chức năng & Nghiệp vụ`, () => {
  test('UM-FN-001: Nút Hủy từ tạo mới quay lại danh sách', async ({ page }) => {
    await navigateTo(page, CREATE_URL);
  ...
  test('UM-FN-002: Validation lỗi bắt buộc khi lưu form trống', async ({ page }) => {
    await navigateTo(page, CREATE_URL);
