...
const MODULE_NAME = 'Internal Order';
const LIST_URL = '/oms/internalOrder';
...
test.describe(`${MODULE_NAME} — Giao diện`, () => {
  test('IO-UI-001: Trang danh sách tải đúng bố cục', async ({ page }) => {
    ...
    await expect(page.locator('text=Internal Order').or(page.locator('text=Internal Orders')).or(page.locator('h1, h2, h3, .title, .header')).first()).toBeVisible();
  });
});
