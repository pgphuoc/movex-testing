...
const LIST_URL = '/tms/podManagementList';
...
test.describe(`${MODULE_NAME} — Giao diện`, () => {
  test('POD-UI-001: Trang Proof of Delivery tải đúng bố cục', async ({ page }) => {
    ...
    await expect(page.locator('text=Proof of Delivery').or(page.locator('text=POD')).first()).toBeVisible();
    await expect(
      page.getByRole('button', { name: /add pod/i })
